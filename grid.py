...
class Grid:
    ...
    def set_start(self, row, col):
        # Moves the start node to a new valid location
        if self.is_valid(row, col) and self.cells[row][col] != GOAL:
            self.cells[self.start[0]][self.start[1]] = EMPTY
            self.start = (row, col)
            self.cells[row][col] = START

    def set_goal(self, row, col):
        # Moves the goal node to a new valid location
        if self.is_valid(row, col) and self.cells[row][col] != START:
    ...
    def spawn_dynamic_obstacle(self, current_path, spawn_probability=0.05):
        # Randomly places a new wall during dynamic mode. 
        # MODIFIED: It can now explicitly spawn ON the current path to trigger replanning.
